...
use horizon_core::{ClickTarget, Panel};
...
pub(super) fn handle_terminal_pointer_input(
    ...
    // combined with the geometric rect test this only flags true occlusion
    // (pointer inside the body rect but claimed by a layer above), not a
    // pointer that simply left the body this frame.
    let pointer_in_body_rect = ui
        .input(|input| input.pointer.latest_pos())
        .is_some_and(|pos| interaction.layout.body.contains(transform_pos(from_global, pos)));
    let body_contains_pointer = interaction.body.contains_pointer() || !pointer_in_body_rect;
    let Some(terminal_mode) = panel.terminal_mut().map(|terminal| terminal.mode()) else {
        return;
    };
    let mut frame_events = PointerFrameEvents::collect(&events, from_global, interaction.layout.body, terminal_mode);
    if !body_contains_pointer {
        frame_events.body_primary_press_pos = None;
        frame_events.body_middle_press_pos = None;
    }
    ...
    for (index, event) in events.iter().enumerate() {
        ...
        match event {
            ...
            egui::Event::PointerMoved(pos) => {
                ...
                if pointer.body_contains_pointer {
...
mod tests {
    use super::{
        pointer_buttons_at_frame_start, raw_press_interpreted, selection_copy_completed, should_request_primary_paste,
