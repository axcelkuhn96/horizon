...
use super::{
    MicState, PanelFrame, clip_screen_rect_to_canvas, mic_accessibility_label, mic_control_enabled,
    mic_control_response, mic_widget_info,
};
...
fn collapse_caret_sits_left_of_close_within_titlebar() {
    ...
    // The mic slot sits left of the caret and must not overlap it.
    assert!(frame.mic.max.x <= frame.collapse.min.x + f32::EPSILON);
}
