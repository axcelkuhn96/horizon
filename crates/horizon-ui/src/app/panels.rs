...
struct PanelFrame {
    ...
    close: Rect,
    collapse: Rect,
    /// Mic control slot left of the collapse caret; only interactive/painted
...
impl PanelFrame {
    fn new(panel_rect: Rect) -> Self {
        ...
        // Mic control sits left of the collapse caret; 22 px spacing keeps the
        // 4 px-expanded hit targets disjoint.
        let mic = Rect::from_center_size(
            Pos2::new(
                collapse.center().x - 22.0,
                panel_rect.min.y + PANEL_TITLEBAR_HEIGHT * 0.5,
            ),
            Vec2::splat(16.0),
        );
