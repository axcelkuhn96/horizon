...
impl HorizonApp {
    ...
    // `bootstrap` is moved field-by-field below rather than destructured up front
    // to keep this constructor under clippy's `too_many_lines` budget without an
    // `#[allow]` (banned for UI source per AGENTS.md).
    fn initial_state(config: &Config, bootstrap: AppBootstrap) -> Self {
        Self {
            ...
            resolved_theme: bootstrap.resolved_theme,
            theme_applied: false,
            panel_screen_rects: HashMap::new(),
            panel_screen_order: Vec::new(),
            ...
            session_store: bootstrap.session_store,
            active_session: None,
            ...
            next_surge_update_check_at: bootstrap.next_surge_update_check_at,
            pending_preset_pick: None,
            dir_picker: None,
            command_palette: None,
            search_overlay: None,
            action_commands_cache: bootstrap.action_commands_cache,
